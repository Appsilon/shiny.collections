...
ui <- shinyUI(fluidPage(
  ...
  div(style="display:inline-block", textInput("unText", "Username", width='200px')),
  uiOutput("chatbox"),
  div(style="display:inline-block", textInput("msgText", "Your message", width='500px')),
...
server <- shinyServer(function(input, output, session) {
  ...
  updateTextInput(session, "unText", value = paste0("User", round(runif(1, 10000, 99999))))
  observeEvent(input$send, {
    shiny.collections::insert(chat, list(user = input$unText, text = input$msgText, time = Sys.time()))
